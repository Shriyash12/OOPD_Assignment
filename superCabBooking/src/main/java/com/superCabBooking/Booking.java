		...
		viewBalance.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (balance.next()) {
						ToastMessage toastMessage = new ToastMessage("WALLET HAS Rs." + balance.getString("MONEY"),
								3000);// showing balance in the wallet
		...
		addMoney.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					stmt.execute("USE CABBOOKING");
					stmt.executeUpdate(
							"UPDATE USERS SET MONEY=MONEY+" + walletMoney.getText() + " WHERE USERID=" + userID);// Adding
																													// money
																													// to
																													// Wallet
		...
		bookButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					ResultSet userFree = stmt.executeQuery(
							"SELECT * FROM USERS WHERE ((BusyTill IS NULL) OR (now() > BusyTill)) AND USERID="
									+ userID);
					if (moneyInWallet < 300.0 || !userFree.next()) {// checking min money in the wallet and also
																	// checking whether user already in a ride
						if (moneyInWallet < 300.0) {
							ToastMessage toastMessage = new ToastMessage("INSUFFICIENT WALLET MONEY,MINIMUM IS Rs.300",
									3000);
							toastMessage.setVisible(true);
						}
						if (!userFree.next()) {
						...
						if (trips == 5) {
							ResultSet noAvailableDriverSet = stmt.executeQuery(
									"SELECT NAME FROM DRIVERS WHERE (BusyTill IS NULL) OR (now() > BusyTill)");
							...
							case 2:
								stmt.executeUpdate("UPDATE DRIVERS SET LATITUDE=10.0,LONGITUDE=20.0 WHERE NAME=\""
										+ driverNames[0] + "\"");
								stmt.executeUpdate("UPDATE DRIVERS SET LATITUDE=13.0,LONGITUDE=23.0 WHERE NAME=\""
										+ driverNames[1] + "\"");
								break;
							case 3:
								stmt.executeUpdate("UPDATE DRIVERS SET LATITUDE=10.0,LONGITUDE=20.0 WHERE NAME=\""
										+ driverNames[0] + "\"");
								stmt.executeUpdate("UPDATE DRIVERS SET LATITUDE=11.0,LONGITUDE=21.0 WHERE NAME=\""
										+ driverNames[1] + "\"");
								stmt.executeUpdate("UPDATE DRIVERS SET LATITUDE=13.0,LONGITUDE=23.0 WHERE NAME=\""
										+ driverNames[2] + "\"");
								break;
							case 4:
								...
								stmt.executeUpdate("UPDATE DRIVERS SET LATITUDE=11.0,LONGITUDE=21.0 WHERE NAME=\""
										+ driverNames[1] + "\"");
								stmt.executeUpdate("UPDATE DRIVERS SET LATITUDE=12.0,LONGITUDE=22.0 WHERE NAME=\""
										+ driverNames[2] + "\"");
								stmt.executeUpdate("UPDATE DRIVERS SET LATITUDE=13.0,LONGITUDE=23.0 WHERE NAME=\""
										+ driverNames[3] + "\"");
								break;
						...
						ResultSet pickUpData = stmt.executeQuery(
								"SELECT * FROM LOCATIONS WHERE NAME=\"" + pickupPoint.getSelectedItem() + "\"");
						if (pickUpData.next()) {
							lat1 = Double.parseDouble(pickUpData.getString("LATITUDE"));
							long1 = Double.parseDouble(pickUpData.getString("LONGITUDE"));
						}
						ResultSet dropData = stmt.executeQuery(
								"SELECT * FROM LOCATIONS WHERE NAME=\"" + dropPoint.getSelectedItem() + "\"");
						if (dropData.next()) {
						...
						Double Fare = (travelDist / 100000.0) * 100; // in rupees , assuming 100rupees per miles
						Double Duration = (travelDist / 1000.0) / 1000; // Duration in hours, assuming 1000Kmph as speed
						String driverName = "", VehicleNum = "", phoneNum = "";

						ResultSet AvailableDrivers = stmt
								.executeQuery("SELECT * FROM DRIVERS WHERE (BusyTill IS NULL) OR (now() > BusyTill)");// checking
																														// for
																														// busyfree
																														// drivers
																														// who
						...
						if (AvailableDrivers.next()) {
							minDistance = distance(lat1, Double.parseDouble(AvailableDrivers.getString("LATITUDE")),
									long1, Double.parseDouble(AvailableDrivers.getString("LONGITUDE")), 0.0, 0.0);
						} else {
						...
						while (AvailableDrivers.next()) {
							if (distance(lat1, Double.parseDouble(AvailableDrivers.getString("LATITUDE")), long1,
									Double.parseDouble(AvailableDrivers.getString("LONGITUDE")), 0.0,
									0.0) < minDistance) {
								minDistance = distance(lat1, Double.parseDouble(AvailableDrivers.getString("LATITUDE")),
										long1, Double.parseDouble(AvailableDrivers.getString("LONGITUDE")), 0.0, 0.0);
							}
						...
						AvailableDrivers = stmt
								.executeQuery("SELECT * FROM DRIVERS WHERE (BusyTill IS NULL) OR (now() > BusyTill)");// drivers
																														// with
																														// max
																														// rating
						while (AvailableDrivers.next()) {
						...
						AvailableDrivers = stmt
								.executeQuery("SELECT * FROM DRIVERS WHERE (BusyTill IS NULL) OR (now() > BusyTill)");
						while (AvailableDrivers.next()) {
							if (distance(lat1, Double.parseDouble(AvailableDrivers.getString("LATITUDE")), long1,
									Double.parseDouble(AvailableDrivers.getString("LONGITUDE")), 0.0,
									0.0) == minDistance) {
								stmt = conn.createStatement();
								ResultSet ChooseFromThis = stmt.executeQuery(
										"SELECT * FROM DRIVERS WHERE ((BusyTill IS NULL) OR (now() > BusyTill)) AND (RATING="
												+ maxRating + ") AND NAME=\"" + AvailableDrivers.getString("NAME")
												+ "\"");// busyfree drivers with min distance and max rating
						...
						// Booking over
						if (DriversAvailable) {
							stmt.executeUpdate("UPDATE DRIVERS SET BusyTill=INTERVAL " + Duration.intValue()
									+ " HOUR+INTERVAL " + minutes.intValue() + " MINUTE+INTERVAL "
									+ inSeconds.intValue() + " SECOND+NOW() WHERE NAME=\"" + driverName + "\"");// updating
																												// busytill
																												// for
																												// driver
							stmt.executeUpdate("UPDATE DRIVERS SET LATITUDE=" + lat2 + ",LONGITUDE=" + long2
									+ " WHERE NAME=\"" + driverName + "\"");// upadating driver location after trip
							stmt.executeUpdate("UPDATE USERS SET MONEY=MONEY-" + Fare + " WHERE USERID=" + userID);// money
																													// deduction
																													// from
																													// user
							stmt.executeUpdate("UPDATE USERS SET BusyTill=INTERVAL " + Duration.intValue()
									+ " HOUR+INTERVAL " + minutes.intValue() + " MINUTE+INTERVAL "
									+ inSeconds.intValue() + " SECOND+NOW() WHERE USERID=" + userID);// updating
																										// busytill for
																										// driver
							stmt.executeUpdate("UPDATE DRIVERS SET TRIPS=TRIPS+1 WHERE NAME=\"" + driverName + "\"");
							Summary MyResultsPage = new Summary(driverName, VehicleNum, phoneNum, Fare,
									Duration.intValue(), minutes.intValue(), maxRating, travelDist);
